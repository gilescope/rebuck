...
| warm floor | **~7 min** | run 29198759978: 99% cache hits, 15 remote commands/leg — the steady state that banking reliability protects |
| archive-unpack transition | 22m29s | run 29200907975 (one-time config move): unpacks now configuration-independent (`archive-unpack` config, linux-pinned exec via the prelude transition in buck2-fixups); surfaced + fixed the case-collision class (linux-preserved README.md/readme.md twins vs NTFS — store now replaces last-wins). Cross-leg dedupe PARTIAL: the per-leg isolation dir still prefixes output paths, so cold archives unpack once per leg, not once per fleet. Full dedupe needs the single-daemon/three-platform invocation experiment. |
| **single daemon** | build **5m31s** | run 29216377986: one buck2 daemon, one buck-out, three concurrent `--target-platforms` builds (legs linux 1m11s / mac 2m49s / win 5m15s), 8/8 bank — cross-leg unpack dedupe complete by construction. Cost of getting here: a cross-platform affinity deadlock (content-based paths carry no config hash → one key spanned OSes → owner could never pull the job; keys now platform-scoped, and a queue-starvation watchdog screams after 120s) and a stale-dice-seed trap (the snapshot seed didn't hash the prelude patch, so a prelude-only change built with old analysis — seed now covers it). Toolchains carry native-compilation exec anchors so per-target routing survives a single all-platform registration. |
